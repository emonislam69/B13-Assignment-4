...
Here is the differences between them-
preventDefault(): Stops the default behavior of the browser (e.g., prevents a link from opening or a form from submitting).
stopPropagation(): Stops the event from bubbling up to parent elements (prevents the parent's event handlers from firing).
